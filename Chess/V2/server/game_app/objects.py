  ...
  def move(self,file,rank,type):
    ...
    if self.game.noLegalMoves(self.owner^1):
      if self.game.inCheck(self.owner^1):
        Debug.out ("No legal moves : " + ("White" if self.owner is 0 else "Black") + " Wins by Checkmate")
        self.game.declareWinner(self.game.players[self.owner],("White" if self.owner is 0 else "Black") + " Wins by Checkmate")
      else:
        Debug.out ("No legal moves : " + ("White" if self.owner is 0 else "Black") + " Stalemated")
  ...
  def makeMove(self,file,rank,type):  
    ...
    if verifyVal is True:
      ...
      if self.type == ord('P') and board[rank-1][file-1] is None and self.file is not file:
        #EnPassant Code  
        for piece in [i for i in self.game.objects.values() if isinstance(i,Piece)]:
          if piece.file == file and abs(piece.rank - rank) is 1:
            targetPiece = piece
    ...
    #Move logic for a pawn
      ...
      if board[rank-1][file-1] is None:
        ...
        if self.file is not file:
          ...
          if abs(self.file-file) is not 1:
            return  ("White" if self.game.playerID is 0 else "Black") + " Attempted to move a pawn more than one square horizontally"
          elif not (rank+2*(self.owner-1) < 8 and rank+2*(self.owner-1) >= 0 and file-1 < 8 and file-1 >= 0):
            return ("White" if self.game.playerID is 0 else "Black") + " Attempted to attack an empty square"
